...
**Дві теки компонентів — це навмисно, і вони не перетинаються.**

`src/components` — робочий застосунок з усією його логікою: станами, валідацією, курсом, клавіатурною навігацією. Storybook його не показує.

`src/storybook` — презентаційна бібліотека, зібрана один-в-один із фрейму `components` у Figma. Компоненти тут без стану, без обробників і без логіки: **вигляд задається пропсом `state`, а не псевдокласом**. `state="hover"` вмикає клас, а не `:hover`.

Так зроблено після того, як підхід «показувати стани застосунку через `storybook-addon-pseudo-states`» виявився ненадійним: аддон не переписує правила з CSS-модулів, а `:focus` не застосовується, поки документ не має фокуса — саме в такому стані знімає хмарний браузер Chromatic. Тепер знімок детермінований за побудовою.

`.storybook/main.ts` читає stories **тільки** з `src/storybook/**`.
